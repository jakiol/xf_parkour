...
//        BotsdkApplication.exitApp();
    ...
    public boolean IsRegisterFailedOrSucceed = false;

    @Override
    public void onRegisterFailed(int i) {
        Log.i("BotSdk", "注册失败，" + i);
//        Toast.makeText(ContextUtil.getContext(), "注册失败", Toast.LENGTH_LONG).show();
        if (IsRegisterFailedOrSucceed == false && _BotInitCallBack != null) {
            _BotInitCallBack.Failed();
        }
        IsRegisterFailedOrSucceed = true;
    }
    ...
    public BotInitCallBack _BotInitCallBack;
