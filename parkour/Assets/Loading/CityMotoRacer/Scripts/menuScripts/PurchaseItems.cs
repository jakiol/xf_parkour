...
public class PurchaseItems : MonoBehaviour
{
		...
		//public GameObject insufficentCoins;
      	public GameObject mainmenu,Storemenu,iap;
	...
	void Update ()
	{
			...
			if (Physics.Raycast (rayObj, out hitObject)) {
				...
				case "BuyNitrous":
					if (PlayerPrefs.GetInt ("TotalCoins", 0) >= NitrousCost) {
				...
				case "BuyMagnet":
					print ("hit Obj");
					if (PlayerPrefs.GetInt ("TotalCoins", 0) >= MagnetCost && PlayerPrefs.GetInt ("MagnetCount", 0) <= 3) {
					...
					} else if (PlayerPrefs.GetInt ("TotalCoins", 0) < MagnetCost){
						iap.SetActive(true);
					}
										break;
								case "OkayBtn":
